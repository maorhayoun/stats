...
import (
	...
	"github.com/maorhayoun/stats/helpers"
...
var (
	...
	timeFormat     = flag.String("date-format", "200601021504", "time resolution of metrics (hour)")
)
...
func main() {
	...
	m.Post("/:key", func(pool *redis.Pool, params martini.Params, req *http.Request, r render.Render, timeFormat string) {
		key := params["key"]
		...
		tkey := time.Now().Truncate(time.Hour).Format(timeFormat)
	...
	m.Get("/:key/recent", func(pool *redis.Pool, params martini.Params, req *http.Request, r render.Render) {
		...
		hours := helpers.ParseHours(timespan)
